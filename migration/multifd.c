...
bool multifd_recv(void)
{
    ...
    for (i = next_recv_channel;; i = (i + 1) % migrate_multifd_channels()) {
         ...
         * only set by this function below. Missing the transition to
         * false is not an issue because it would only send us looking
    ...
    assert(!qatomic_xchg(&p->pending_job, true));
    qemu_sem_post(&p->sem);
