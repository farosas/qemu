...
static inline void powerpc_excp(PowerPCCPU *cpu, int excp)
{
    ...
    PPCIntrArgs regs;
    int excp_model, lev = -1;

    if (!intr_model) {
        error_report("No interrupt model registered! Aborting");
        exit(1);
    }

    excp_model = intr_model->id;
    ...
    regs.new_nip = intr_state->excp_vectors[excp];
    if (regs.new_nip == (target_ulong)-1ULL) {
        cpu_abort(cs, "Raised an exception without defined vector %d\n",
                  excp);
    }

    regs.new_nip |= intr_state->excp_prefix;
