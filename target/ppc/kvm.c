...
int kvm_arch_insert_sw_breakpoint(CPUState *cs, struct kvm_sw_breakpoint *bp)
{
    ...
    unsigned long addr = bp->pc;
    SpaprMachineState *spapr = SPAPR_MACHINE(qdev_get_machine());

    if (!spapr->vof_client_started)
	    addr += spapr->kernel_addr;

    if (cpu_memory_rw_debug(cs, addr, (uint8_t *)&bp->saved_insn,
                            sizeof(sc), 0) ||
        cpu_memory_rw_debug(cs, addr, (uint8_t *)&sc, sizeof(sc), 1)) {
        return -EINVAL;
    }
...
int kvm_arch_remove_sw_breakpoint(CPUState *cs, struct kvm_sw_breakpoint *bp)
{
    ...
    if (cpu_memory_rw_debug(cs, addr, (uint8_t *)&sc, sizeof(sc), 0) ||
        sc != debug_inst_opcode ||
        cpu_memory_rw_debug(cs, addr, (uint8_t *)&bp->saved_insn,
                            sizeof(sc), 1)) {
        return -EINVAL;
    }
...
void kvm_arch_emulate_singlestep(CPUState *cs, int enabled)
{
    ...
    if (rfid) {
        ...
        mmu_on = ((env->spr[SPR_SRR1] >> MSR_IR) & 1 &&
                  (env->spr[SPR_SRR1] >> MSR_DR) & 1);
    } else {
        env->msr |= (1ULL << MSR_SE);
        mmu_on = (FIELD_EX64(env->msr, MSR, DR) &&
...
void kvm_singlestep_ail_change(CPUState *cs)
{
    ...
    kvm_insert_singlestep_breakpoint(cs, (FIELD_EX64(env->msr, MSR, DR) &&
					  FIELD_EX64(env->msr, MSR, IR)));
}
