...
void kvm_arch_emulate_singlestep(CPUState *cs, int enabled)
{
     ...
     */
    if (rfid) {
        if (!env->pending_singlestep && (env->spr[SPR_SRR1] >> MSR_SE) & 1) {
            /*
             * The guest is doing a single step itself. Make sure we
             * restore it later.
             */
            env->guest_singlestep = true;
        }
...
static int kvm_handle_sw_breakpoint(CPUState *cs, target_ulong address)
{
    ...
    if (address == env->trace_handler_addr && env->pending_singlestep) {
        /*
         * A pending single step means that we stepped an instruction
         * that caused an interrupt and then hit a breakpoint inside
         * the handler. Although we have singlestep_enabled == false
         * by now, the original step still wasn't handled.
         */
        cs->singlestep_enabled = true;
        env->pending_singlestep = false;
    ...
    CPU_FOREACH(cs) {
        ...
        if (kvm_find_sw_breakpoint(cs, env->trace_handler_addr) &&
