...
void kvm_arch_set_singlestep(CPUState *cs, int enabled)
{
    ...
    if (enabled) {
         ...
         * QEMU. */
        trace_handler_addr = ppc_get_interrupt_handler_addr(cs,
                                                            POWERPC_EXCP_TRACE);
                                 ...
                                 struct kvm_debug_exit_arch *arch_info)
{
    ...
    cpu_synchronize_state(cs);
    trace_handler_addr = ppc_get_interrupt_handler_addr(cs, POWERPC_EXCP_TRACE);
    ...
    if (arch_info->address == trace_handler_addr) {
        ...
        env->msr = env->spr[SPR_SRR1] & ~(1ULL << MSR_SE | (PPC_BITMASK(33, 36) |
                                                            PPC_BITMASK(43, 47)));
                                    ...
                                    struct kvm_debug_exit_arch *arch_info)
{
    ...
    cpu_synchronize_state(cs);
    trace_handler_addr = ppc_get_interrupt_handler_addr(
        cs, POWERPC_EXCP_TRACE);
